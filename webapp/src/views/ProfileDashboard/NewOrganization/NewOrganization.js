...
import { Button, Form, Input, Layout, PageHeader, Radio, Spin, Tooltip, Typography } from 'antd';
  ...
  const handleSubmit = (event) => {
    ...
    if (user.email_verified) {
      post(`/organization/new-organization`, {
        ...
        .then(() => {
          dispatch(createAlert('Organization Added', 'Organization has been successfully added.', 'success', true));
          setLoading(false);
          setRedirect(true);
        })
        .catch(() => {
        ...
        <Form {...formItemLayout}>
          ...
          <div style={{ width: '100%', textAlign: 'center' }} onClick={(e) => handleSubmit(e)}>
            {loading ? (
              ...
              <Button disabled={!validOrgName || !radioChecked} type="primary" id="create-org-btn">
                Create Organization
              </Button>
            )}
          </div>
