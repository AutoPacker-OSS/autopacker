...
function Servers() {
  ...
  useEffect(() => {
    ...
    if (debouncedSearchTerm) {
      ...
      get(`/server/${search}`).then((resp) => setServers(resp));
    } else {
      get(`/server`).then((resp) => setServers(resp));
    }
  }, [debouncedSearchTerm]);
      ...
      <Content
        ...
        <div style={{ display: 'flex' }}>
          ...
          <Button
            ...
            type="primary">
            <Link id="new-server-link" to="/profile/servers/add">
              New Server
            </Link>
          </Button>
        </div>
        <Row style={{ marginTop: 20 }} gutter={[24, 24]}>
          {servers.map((server) => (
            <Col xs={24} key={server.serverId}>
              <ServerRow id={server.serverId} title={server.title} desc={server.description} status={server.status} />
            </Col>
          ))}
        </Row>
      </Content>
