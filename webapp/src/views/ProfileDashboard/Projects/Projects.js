...
function Projects() {
  ...
  useEffect(() => {
    ...
    if (debouncedSearchTerm) {
      ...
      get(`/projects/${user.username}/search?q=${search}`).then((resp) => setProjects(resp));
    } else {
      get(`/projects`).then((resp) => setProjects(resp));
    }
  }, [debouncedSearchTerm]);
        ...
        <Row style={{ marginTop: 20 }} gutter={[24, 24]}>
          {projects.map((project) => (
            <Col xs={24} lg={8} xl={6} xxl={4} key={project.id}>
              {selectedCard !== null ? <Navigate to={'/profile/projects/overview/' + selectedCard} /> : <div />}
