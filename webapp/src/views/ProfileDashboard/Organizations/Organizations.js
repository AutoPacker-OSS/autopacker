...
function Organizations() {
  ...
  useEffect(() => {
    ...
    if (debouncedSearchTerm) {
      ...
      get(`/organization/${user.username}/isMember/search?q=${search}`).then((resp) => setOrganizations(resp));
    } else {
      get(`/organization/${user.username}/isMember`).then((resp) => setOrganizations(resp));
    }
  }, [debouncedSearchTerm]);
      ...
      <Content
        ...
        <div style={{ display: 'flex' }}>
          ...
          <Button
            ...
            type="primary">
            <Link id="new-organization-link" to="/profile/organization/add">
              New Organization
            </Link>
          </Button>
        </div>
        <Row style={{ marginTop: 20 }} gutter={[24, 24]}>
          {organizations.map((organization) => (
            <Col xs={24} lg={8} xl={6} key={organization.id}>
              {selectedCard !== null ? <Navigate to={'/organization/dashboard/' + selectedCard} /> : <div />}
