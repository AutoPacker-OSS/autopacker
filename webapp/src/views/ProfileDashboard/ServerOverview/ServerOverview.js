...
function ServerOverview() {
  ...
  const toggleLoadingState = (projectId) => {
    ...
    projectsLoading.forEach((project) => {
      if (project.id === projectId) {
        projectsLoading[projectsLoading.indexOf(project)].loading =
          !projectsLoading[projectsLoading.indexOf(project)].loading;
      }
    });
  ...
  return (
    <div style={{ width: '100%' }}>
      <PageHeader
        ...
        {server.description !== null ? <Paragraph>{server.description}</Paragraph> : <div />}
        ...
        <div>
          <Paragraph style={{ color: statusColor }}>Status: Unavailable{/*server.status*/}</Paragraph>
          <Paragraph>
            Username: {server.serverUsername} - {/*Default Password: {server.password} - */}
      ...
      <Content
          ...
          <Row gutter={[24, 24]}>
            {serverProjects.map((project) => (
              <Col xs={24} lg={8} xl={6} key={serverProjects.indexOf(project)}>
                <Spin spinning={getLoadingStateForProject(project.id) && !deployFinished}>
                  <Card
                    ...
                    actions={[
                      ...
                      />,
                      <DeleteOutlined key="delete" onClick={() => selectModalToDelete(project.id, project.name)} />,
                    ]}>
                    <div onClick={() => setModalOpen(true)}>
                      ...
                      <Meta
                        style={{ padding: 20 }}
                        title={project.name}
                        description={<Paragraph ellipsis>{project.description}</Paragraph>}
                      />
                    </div>
                  </Card>
                </Spin>
                ...
                <Card
                  ...
                  <div>
                    ...
                    <div style={{ verticalAlign: 'middle' }}>
                      <Text style={{ marginLeft: 16 }}>{project.name}</Text>
                      <Checkbox checked={getProjectCheckedValue(project.id)} style={{ float: 'right' }} />
                    </div>
                  </div>
                </Card>
              ))
                ...
                serverProjects.includes(project) ? (
                  <Card
                    ...
                    <div>
                      ...
                      <div style={{ verticalAlign: 'middle' }}>
                        <Text style={{ marginLeft: 16 }}>{project.name}</Text>
                        <Checkbox checked={true} style={{ float: 'right' }} />
                      </div>
                    </div>
                  </Card>
                ) : (
          ...
          <Modal
            ...
            <Form layout="vertical">
              <Form.Item
                ...
                <Input.Password onChange={(e) => setServerPassword(e.target.value)} />
              </Form.Item>
            </Form>
          </Modal>
        ) : (
        ...
        {installationModalOpen ? (
          <Modal
            ...
            <Paragraph>
              This will disable the server firewall, and download docker and docker-compose. These are needed to be able
              to run projects on the server.
            </Paragraph>
            <Form layout="vertical">
              <Form.Item
                name="password"
        ...
        {deployProjectModalOpen ? (
          <Modal
            ...
            <Paragraph>Fill in your server password to connect to server and click ok to deploy project</Paragraph>
            <Form layout="vertical">
              <Form.Item
                name="password"
