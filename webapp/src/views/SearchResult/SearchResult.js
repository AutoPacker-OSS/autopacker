...
function SearchResult() {
  ...
  useEffect(() => {
    ...
    const usersUrl = process.env.REACT_APP_APPLICATION_URL + process.env.REACT_APP_API + '/auth/search?q=' + query;
              ...
              <Tabs defaultActiveKey={'1'}>
                ...
                <TabPane tab={'Organizations (' + organizationsList.length + ')'} key="3">
                  {organizationsList.length <= 0 ? (
                    ...
                    <Row gutter={[0, 12]}>
                      <Divider />
                      {organizationsList.slice(minNumbOrganizations, maxNumbOrganizations).map((organization) => (
