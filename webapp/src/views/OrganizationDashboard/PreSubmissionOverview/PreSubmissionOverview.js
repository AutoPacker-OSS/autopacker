...
function PreSubmissionOverview() {
  ...
  const handleSubmit = (event) => {
    ...
    if (keycloak.idTokenParsed.email_verified) {
      axios({
        method: 'post',
        url: process.env.REACT_APP_APPLICATION_URL + process.env.REACT_APP_API + '/organization/submitProject',
        headers: {
          Authorization: authState.accessToken !== null ? `Bearer ${authState.accessToken}` : undefined,
        },
        data: {
        ...
        .then(function () {
          ...
          axios({
            method: 'post',
            url: process.env.REACT_APP_APPLICATION_URL + process.env.REACT_APP_API + '/organization/submitted',
            headers: {
              Authorization: authState.accessToken !== null ? `Bearer ${authState.accessToken}` : undefined,
            },
            data: {
        ...
        .catch(() => {
          dispatch(createAlert('Project Submission Failed', 'Submission failed, please try again', 'error', true));
        });
    } else {
      dispatch(
        createAlert(
  ...
  return (
    <div style={{ width: '100%', height: 'auto' }}>
      ...
      <PageHeader
        ...
        extra={[
          <Button key="submit-project" onClick={() => setSubmitModalStatus(true)}>
            Submit
          </Button>,
          <Button danger key="delete-project" onClick={() => setDeleteModalStatus(true)}>
            Delete
          </Button>,
        ]}>
      ...
      <Content
        ...
        {projectModules.length > 0 ? (
          ...
          <Empty
            ...
            description="Project contains no modules"></Empty>
        )}
        {selectedModule !== null ? (
          <Modal
