...
function Submissions() {
  ...
  const deleteRequest = (applicationId) => {
    _delete(`/server/${organizationName}/delete-project-application/${applicationId}`)
      .then(() => {
        dispatch(createAlert('Request Deleted', 'You have successfully deleted the project request', 'success', true));
        toggleRefresh();
        setOpenDeleteModal(false);
      })
      .catch(() => {
        dispatch(createAlert('Request Deletion Failed', 'Failed to delete the project request', 'error', true));
        setOpenDeleteModal(false);
      });
  };
  ...
  return (
    <div style={{ width: '100%' }}>
      <PageHeader
        ...
        breadcrumb={{ routes }}>
        <Paragraph>List of all the project submissions you have with this organization</Paragraph>
      </PageHeader>
        ...
        <div
          ...
          <Collapse>
            {requests.map((request) => (
              <Panel
                header={request.project.owner.username + ' - ' + request.project.name}
                key={request.id}
                ...
                <Row gutter={[24, 0]}>
                  ...
                  <Col xs={24} md={19}>
                    <Descriptions title="Project Application Details" layout="horizontal">
                      <Descriptions.Item label="Submitted By">{request.project.owner.username}</Descriptions.Item>
                      <Descriptions.Item label="Project Name">{request.project.name}</Descriptions.Item>
                      {/* TODO Removed? */}
                      ...
                      <Descriptions.Item label="Tags">
                        {request.project.tags !== null && request.project.tags.length > 1 ? (
                          request.project.tags.split(',').map((tag) => (
                    ...
                    <Descriptions layout="horizontal">
                      <Descriptions.Item label="Description">{request.project.description}</Descriptions.Item>
                    </Descriptions>
                    ...
                    <Descriptions layout="horizontal">
                      <Descriptions.Item label="Comment">{request.comment}</Descriptions.Item>
                    </Descriptions>
          ...
          {selectedRequest !== null ? (
            <Modal
              title={'Delete "' + selectedRequest.project.name + '" request?'}
              centered
