...
function ProfileOrganizationOverview() {
  ...
  useEffect(() => {
    if (organizationName) {
      ...
      get(`/organization/${organizationName}`).then((resp) => setOrganization(resp));
  ...
  useEffect(() => {
    ...
    if (debouncedSearchTerm) {
      get(`/organization/${organization.name}/projects/search?q=${search}`).then((resp) => setProjects(resp));
    } else {
      const organizationName = sessionStorage.getItem('selectedPublicOrganization');

      get(`/organization/${organizationName}/projects`).then((resp) => setProjects(resp));
    }
  }, [debouncedSearchTerm]);
                  ...
                  isAuthenticated ? (
                    user?.email_verified && !isMember ? (
                      <Button key="application" onClick={() => sessionStorage.removeItem('isMember')}>
                        <Link to={'/organization/' + organization.name + '/membership'}>Request Membership</Link>
                      </Button>
                    ) : (
                      <div />
                    )
